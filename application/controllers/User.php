...

{
  ...
  public function error404() {
...

    {
      ...
      $data['historyId'] = isset($_POST['historyId'])?$_POST['historyId']:'';
      $data['val'] = isset($_POST['val'])?$_POST['val']:'';
      $data['step'] = isset($_POST['step'])?$_POST['step']:'';
      $data['exiting_profile'] = isset($_POST['exiting_profile'])?$_POST['exiting_profile']:'';
      $data['primary_profile'] = isset($_POST['primary_profile'])?$_POST['primary_profile']:'';
       $data['h1'] = isset($_POST['h1'])?$_POST['h1']:'';
        $data['h2'] = isset($_POST['h2'])?$_POST['h2']:'';
         $data['h3'] = isset($_POST['h3'])?$_POST['h3']:'';
...
if ($npkewording == "Ελάχιστη" && $npkdawording == "Ελάχιστη") {    
   ...
   $this->load->view('np/IRMA_analysis',$data); 
...

    {
        ...
        $data['ok_f_no'] = $this->user_model->getnumDataFieldwithwhere('*','customers', 'oikogeniakikatastasi="Χήρα/ος"', 'oikogeniakikatastasi="Widow/er"' );
        $data['ok_g_no'] = $this->user_model->getnumDataFieldwithwhere('*','customers', 'oikogeniakikatastasi="Χήρα/ος με παιδιά"', 'oikogeniakikatastasi="Widow/er with children"' );
       ...
       $data['invesment'] = $this->user_model->get_other_feild('invesment="1"');
        $data['home'] = $this->user_model->get_other_feild('home !="[]"');
        $data['home_ins'] = $this->user_model->get_other_feild('home_ins !="[]"');
        $data['car1'] = $this->user_model->get_other_feild('car1 !="[]"');
        $data['car_ins_d'] = $this->user_model->get_other_feild('car_ins !="[]"');
        $data['small_busines'] = $this->user_model->get_other_feild('small_busines != "[]"');
        $data['bus_ins_d'] = $this->user_model->get_other_feild('bus_ins != "[]"');
        $data['boat'] = $this->user_model->get_other_feild('boat !="[]"');
        $data['boat_ins_d'] = $this->user_model->get_other_feild('boat_ins !="[]"');
        $data['group_ins'] = $this->user_model->get_other_feild('group_ins ="yes"');

        $data['life_ins1'] = $this->user_model->get_other_feild('life_ins1 !="[]"');
         $data['oth_ins'] = $this->user_model->get_other_feild('oth_ins !="[]"');
  ...
  //  }
...

    {
            ...
            $data['customer'] = $this->user_model->getRowData('customers', 'id="'.$id.'"', $join_array = false);
             $data['cusDetails'] = $this->user_model->getRowData('customer_datails', 'cus_id="'.$id.'"', $join_array = false);    
            $data['human'] =  $this->user_model->getDataField('*','hcresults', 'id_pelati="'.$id.'"',$order_by = false, $order = false, $join_array = false, $limit = false, $join_type = false);      
...
//print_r($data);die;
 ...
 if(!empty($_FILES['image']['name'])){
    ...
    //full path of the thumbnail image
            ...
            'report_sufix'=>$this->input->post('report_sufix'),
            'report_name'=>$this->input->post('report_name'),

            'customerstreet'=>$this->input->post('customerstreet'),
            'customerstreetnum'=>$this->input->post('customerstreetnum'),
        ...
        $result = $this->user_model->insertData('customers',$data);
         ...
         $last_id = $this->db->insert_id();
           ...
           // ---------------- Home--------------------------//
            ...
            $filtered_home = array_filter($home); 
            $filtered_home =  json_encode($filtered_home);

            $home_ins = $this->input->post('home_ins');
            $filtered_home_ins = array_filter($home_ins); 
            $filtered_home_ins =  json_encode($filtered_home_ins);

            $home_ins_company = $this->input->post('ins_company');
            $filtered_ins_company = array_filter($home_ins_company); 
            $filtered_ins_company =  json_encode($filtered_ins_company);

            $home_date_renew_home1 = $this->input->post('date_renew_home1');
            $filtered_date_renew_home1 = array_filter($home_date_renew_home1); 
            $filtered_date_renew_home1 =  json_encode($filtered_date_renew_home1);

             // ---------------- Car--------------------------//

            $vehicle = $this->input->post('vehicle');
            $filtered_vehicle = array_filter($vehicle); 
            $filtered_vehicle =  json_encode($filtered_vehicle);

            $car1 = $this->input->post('car1');
            $filtered_car1 = array_filter($car1); 
            $filtered_car1 =  json_encode($filtered_car1);

            $car_ins = $this->input->post('car_ins');
            $filtered_car_ins = array_filter($car_ins); 
            $filtered_car_ins =  json_encode($filtered_car_ins);

            $ins_company_car = $this->input->post('ins_company_car');
            $filtered_ins_company_car = array_filter($ins_company_car); 
            $filtered_ins_company_car =  json_encode($filtered_ins_company_car);

            $date_renew = $this->input->post('date_renew');
            $filtered_date_renew = array_filter($date_renew); 
            $filtered_date_renew =  json_encode($filtered_date_renew);

             // ---------------- busines--------------------------//

            $small_busines = $this->input->post('small_busines');
            $filtered_small_busines = array_filter($small_busines); 
            $filtered_small_busines =  json_encode($filtered_small_busines);

            $bus_ins = $this->input->post('bus_ins');
            $filtered_bus_ins = array_filter($bus_ins); 
            $filtered_bus_ins =  json_encode($filtered_bus_ins);

            $ins_com_bus = $this->input->post('ins_com_bus');
            $filtered_ins_com_bus = array_filter($ins_com_bus); 
            $filtered_ins_com_bus =  json_encode($filtered_ins_com_bus);

            $date_renew_bus = $this->input->post('date_renew_bus');
            $filtered_date_renew_bus = array_filter($date_renew_bus); 
            $filtered_date_renew_bus =  json_encode($filtered_date_renew_bus);

             // ---------------- boat--------------------------//

            $boat = $this->input->post('boat');
            $filtered_boat = array_filter($boat); 
            $filtered_boat =  json_encode($filtered_boat);

            $boat_ins = $this->input->post('boat_ins');
            $filtered_boat_ins = array_filter($boat_ins); 
            $filtered_boat_ins =  json_encode($filtered_boat_ins);

            $ins_com_boat = $this->input->post('ins_com_boat');
            $filtered_ins_com_boat = array_filter($ins_com_boat); 
            $filtered_ins_com_boat =  json_encode($filtered_ins_com_boat);

            $date_renew_boat = $this->input->post('date_renew_boat');
            $filtered_date_renew_boat = array_filter($date_renew_boat); 
            $filtered_date_renew_boat =  json_encode($filtered_date_renew_boat);

             // ---------------- life--------------------------//

            $life_ins = $this->input->post('life_ins');
            $filtered_life_ins = array_filter($life_ins); 
            $filtered_life_ins =  json_encode($filtered_life_ins);

            $life_dis = $this->input->post('life_dis');
            $filtered_life_dis = array_filter($life_dis); 
            $filtered_life_dis =  json_encode($filtered_life_dis);

            $ins_com_life = $this->input->post('ins_com_life');
            $filtered_ins_com_life = array_filter($ins_com_life); 
            $filtered_ins_com_life =  json_encode($filtered_ins_com_life);

            $date_renew_life = $this->input->post('date_renew_life');
            $filtered_date_renew_life = array_filter($date_renew_life); 
            $filtered_date_renew_life =  json_encode($filtered_date_renew_life);

 // ---------------- other ins--------------------------//
             $oth_ins = $this->input->post('oth_ins');
            $filtered_oth_ins = array_filter($oth_ins); 
            $filtered_oth_ins =  json_encode($filtered_oth_ins);

            $oth_dis = $this->input->post('oth_dis');
            $filtered_oth_dis = array_filter($oth_dis); 
            $filtered_oth_dis =  json_encode($filtered_oth_dis);

            $ins_com_oth = $this->input->post('ins_com_oth');
            $filtered_ins_com_oth = array_filter($ins_com_oth); 
            $filtered_ins_com_oth =  json_encode($filtered_ins_com_oth);

            $date_renew_oth = $this->input->post('date_renew_oth');
            $filtered_date_renew_oth = array_filter($date_renew_oth); 
            $filtered_date_renew_oth =  json_encode($filtered_date_renew_oth);
           ...
           $data2 = array(
            ...
            'home'=>$filtered_home,
            'home_ins'=>$filtered_home_ins,
            'ins_company'=>$filtered_ins_company,
            'date_renew_home1'=>$filtered_date_renew_home1,
            
            'vehicle'=>$filtered_vehicle,
            'car1'=>$filtered_car1,
            'car_ins'=>$filtered_car_ins,
            'ins_company_car'=>$filtered_ins_company_car,
            'date_renew'=>$filtered_date_renew,
            
            'small_busines'=>$filtered_small_busines,
            'bus_ins'=>$filtered_bus_ins,
            'ins_com_bus'=>$filtered_ins_com_bus,
            'date_renew_bus'=>$filtered_date_renew_bus,            
           
            'boat'=>$filtered_boat,
            'boat_ins'=>$filtered_boat_ins,
            'ins_com_boat'=>$filtered_ins_com_boat,
            'date_renew_boat'=>$filtered_date_renew_boat,

             'group_ins'=>$this->input->post('group_ins'),  
             'group_desp'=> $this->input->post('group_dis'), 
             'ins_com_grp'=>$this->input->post('ins_com_grp'),
             'date_renew_grp'=>$this->input->post('date_renew_grp'),

             'life_ins1'=>$filtered_life_ins,
             'life_dis1'=>$filtered_life_dis,  
             'ins_com_life1'=>$filtered_ins_com_life, 
             'date_renew_life1'=>$filtered_date_renew_life, 

             'oth_ins'=>$filtered_oth_ins,
             'oth_dis'=>$filtered_oth_dis,  
             'ins_com_oth'=>$filtered_ins_com_oth, 
             'date_renew_oth'=>$filtered_date_renew_oth, 
             'id_asfalisti'=>$this->session->userdata('user_id'), 

                 
                 'cus_id'=>$last_id,             
            );
           $this->db->insert('customer_datails',$data2);
        ...
        redirect("addClient");
         ...
         if($id){
          ...
          $data['total'] = $this->user_model->gettotalmember(); 
         $query = $this->db->get('tbl_insurance_list');
        $data['ins_list'] = $query->result();
        $data['ins_list2'] = $query->result();
        $data['ins_list3'] = $query->result();
        $data['ins_list4'] = $query->result();
        $data['ins_list5'] = $query->result();
        $data['ins_list6'] = $query->result();
        $data['ins_list7'] = $query->result();
        $data['ins_list8'] = $query->result();
        $data['ins_list9'] = $query->result();
        $data['ins_list10'] = $query->result();
        $data['ins_list11'] = $query->result();
...
if($query->num_rows() > 0){
$this->db->where('cus_id',$id);
           $this->db->update('customer_datails',$data2);
  ...
  $this->db->insert('customer_datails',$data2);
        ...
        $data['customer'] = $this->user_model->getRowData('customers', 'id="'.$id.'"', $join_array = false); 
         $data['cdetails'] = $this->user_model->getRowData('customer_datails', 'cus_id="'.$id.'"', $join_array = false);
        $query = $this->db->get('tbl_insurance_list');
        $data['ins_list'] = $query->result();
        $data['ins_list2'] = $query->result();
        $data['ins_list3'] = $query->result();
...
public function report_cal_edit($id,$cid)
    {


      $data['profile'] = $this->user_model->getRowData('customers', 'id="'.$cid.'"', $join_array = false); 
      $data['report'] = $this->user_model->getRowData('hcresults', 'id="'.$id.'"', $join_array = false); 
       $data['id'] = $id; 

      $this->load->view('life_calculator_edit',$data);
    }


    public function delete_calculator_data()
    {
      if($this->input->post('delete_reff')) {

      $cid = $this->input->post('clientid');
      $id = $this->input->post('cid_val');
      $this->db->where('id',$id);
      $this->db->delete('hcresults');
      redirect("clientDetails/$cid");
    }
}

public function delete_pension_arc()
    {
      if($this->input->post('delete_reff')) {
      ...
      $id = $this->input->post('cid_val');
      $this->db->where('id',$id);
      $this->db->delete('pension_arc');
      redirect("clientDetails/$cid");
    }
}

public function delete_education_erc()
    {
      if($this->input->post('delete_reff')) {
...

    {
     ...
     if(isset($_GET['credate'])) {
      ...
      if(isset($_GET['val'])){
       ...
       // $data['qua'] = $this->user_model->getRowData('irmaresults_other', 'irmaresult_id="'.$cid.'"', $join_array = false);
         $data['ans'] = $this->user_model->view_final_analysis_edit($cdate, $cid);  
      }else{
...
$userresult = $this->user_model->getRowData('members', 'id="'.$id_asfalisti.'"', $join_array = false); 
            ...
            //echo $privatesavings.'--'.$othermoainsurance.'--'.$kefaleosiggenwn; die;
            $kam = $ksla + $privatesavings + $othermoainsurance + $kefaleosiggenwn;

            $kam = $ksla + $privatesavings + $life_saving + $kefaleosiggenwn;
            ...
            if ($olikoelaxistopre <= 30000) {
                $olikoelaxisto = 30000  - $life_saving;;
                $olikoelaxistono = number_format($olikoelaxisto, 2);
               // $olikoelaxistono = $olikoelaxistono - $life_saving;
            } elseif ($olikoelaxistopre >=30000) {
                $olikoelaxisto = $olikoelaxistopre  - $life_saving;;
                $olikoelaxistono = number_format($olikoelaxisto, 2);
               // $olikoelaxistono = $olikoelaxistono - $life_saving;
            }
            ...
            $data = array(
              'id_pelati'=>$id,
              'id_asfalisti'=>$userid,
              'forosklironomias'=>$forosklironomias,
              'sumofgeneraldebt'=>$sumofgeneraldebt,
              'sumofgeneraldebtno'=>$sumofgeneraldebtno,
              'remaininghousingdebt'=>$remaininghousingdebt,
              'remaininghousingdebtno'=>$remaininghousingdebtno,
              'housingbankinsurance'=>$housingbankinsurance,
              'payondeath'=>$payondeath,
              'moayears'=>$moayears,
              'olikoelaxistono'=>$olikoelaxistono,
              'minimumfamilysurvivalamount'=>$minimumfamilysurvivalamount,
              'minimumfamilysurvivalamountno'=>$minimumfamilysurvivalamountno,
              'idealfamilyincome'=>$idealfamilyincome,
              'idealfamilyincomeno'=>$idealfamilyincomeno,
              'familysurvivalyears'=>$familysurvivalyears,
              'minimumfamilysurvivaltotalyearsno'=>$minimumfamilysurvivaltotalyearsno,
              'idealfamilyincometotalyearsno'=>$idealfamilyincometotalyearsno,
              'moatotalyearsno'=>$moatotalyearsno,
              'dilothen'=>$dilothen,
              'dilothenno'=>$dilothenno,
              'houseandequipmentcost'=>$houseandequipmentcost,
              'houseandequipmentcostno'=>$houseandequipmentcostno,
              'monthlycare'=>$monthlycare,
              'monthlycareno'=>$monthlycareno,
              'monthlycaretotalno'=>$monthlycaretotalno,
              'montlhycareyears'=>$montlhycareyears,
              'kgankp'=>$kgankp,
              'sinolospoudwnno'=>$sinolospoudwnno,
              'sinolospoudwntotalno'=>$sinolospoudwntotalno,
              'monthlypensionwidow'=>$monthlypensionwidow,
              'monthlypensionwidowno'=>$monthlypensionwidowno,
              'monthlypensionwidowyears'=>$monthlypensionwidowyears,
              'monthlypensionhc'=>$monthlypensionhc,
              'monthlypensionhcyears'=>$monthlypensionhcyears,
              'privatesavings'=>$privatesavings,
              'privatesavingsno'=>$privatesavingsno,
              'life_saving'=>$life_saving,
              'otherinsurance'=>$otherinsurance,
              'otherinsuranceno'=>$otherinsuranceno,
              'othermoainsurance'=>$othermoainsurance,
              'othermoainsuranceno'=>$othermoainsuranceno,
              'offeredincome'=>$offeredincome,
              'offeredincomeno'=>$offeredincomeno,
              'offeredincomeyears'=>$offeredincomeyears,
              'kslxno'=>$kslxno,
              'kefaleosiggenwnno'=>$kefaleosiggenwnno,
              'ka'=>$ka,
              'kam'=>$kam,
              'payondeathvalue'=>$payondeathvalue,
              'sinolikoeksoflisisdaneiwnxrewnno'=>$sinolikoeksoflisisdaneiwnxrewnno,
              'moavalueno'=>$moavalueno,
              'creation_date'=>$currentdate,
              );

            $this->db->insert('hcresults',$data);
...
<p>'.$this->lang->line('KEFALEOALLES').': <strong>'.$privatesavingsno.'</strong>€</p>
<p>'.$this->lang->line('KEFALEOALLESMOA').': <strong>'.$othermoainsuranceno.'</strong>€</p>
<p>'.$this->lang->line('KEFALEOIDIOTIKES').': <strong>'.number_format((float)str_replace(',', '', $life_saving), 2, '.', ',').'</strong>€</p>
 ...
 * @param number $userId : Optional : This is user id
 */
    function calculator_report_edit($id=NULL)
    {     
...
$userresult = $this->user_model->getRowData('members', 'id="'.$id_asfalisti.'"', $join_array = false); 
            ...
            $this->db->where('id',$cal_id);

            $this->db->update('hcresults',$data);

             $this->session->set_flashdata('success','Human Capital Value Calculation Edited Sucessfully');
              redirect("clientDetails/$clientId");
 ...
 * @param number $userId : Optional : This is user id
 */
    function calculator_report_arc()
    {     
 $useridclient = $_GET['mainuserid'];
    ob_clean();
    require_once('tcpdf/tcpdf.php');
    $customer = $this->user_model->getRowData('customers', 'id="'.$useridclient.'"', $join_array = false); 
...
$userresult = $this->user_model->getRowData('members', 'id="'.$userid.'"', $join_array = false); 
    ...
    $createdDate = $_GET['credate'];

    $this->db->select('*');
    $this->db->where('id_pelati',$useridclient);
    $this->db->where('creation_date',$createdDate);
    $query = $this->db->get('hcresults');
    $hcresult = $query->row();

//print_r($hcresult);
     
            // 2η Κάρτα
            $forosklironomias = $hcresult->forosklironomias;
            // Δανεια καταναλωτικα – χρεη
            $sumofgeneraldebt = $hcresult->sumofgeneraldebt; 
            if (empty($sumofgeneraldebt)) {
                $sumofgeneraldebt = 0;
            }
            $rgd = number_format($sumofgeneraldebt, 2);
            ...
            // Δανειο στεγαστικο
            $remaininghousingdebt = $hcresult->remaininghousingdebt;
            $rhd = number_format($remaininghousingdebt, 2);
            ...
            $remaininghousingdebtno = number_format($remaininghousingdebt, 2);
            // Ασφαλισμένο στη Τράπεζα NAI/OXI
            $housingbankinsurance = $hcresult->housingbankinsurance;
            // Αποπληρωμή εφάπαξ σε περίπτωση θανάτου; NAI/OXI
            $payondeath = $hcresult->payondeath;
            $olikoelaxisto = 30000;
            // Καταναλωτικά Δάνεια
            if ($housingbankinsurance == "Ναι") {
                $sumofgeneraldebtno = 0;
            ...
            // 3η Κάρτα
            // Ελαχιστο μηνιαιο εισοδημα επιβιωσης της οικογενειας
            $minimumfamilysurvivalamount = $hcresult->minimumfamilysurvivalamount;
            if (empty($minimumfamilysurvivalamount)) {
                $minimumfamilysurvivalamount = 0;
            }
            $minimumfamilysurvivalamountno = number_format($minimumfamilysurvivalamount, 2);
            ...
            // Ιδανικο εισοδημα επιβιωσης της οικογενεις που προστατευει τον σημερινο τροπο ζωης
            $idealfamilyincome = $hcresult->idealfamilyincome;
            if (empty($idealfamilyincome)) {
                $idealfamilyincome = 0;
            }
            $idealfamilyincomeno = number_format($idealfamilyincome, 2);
            // Για πόσα έτη;
            $familysurvivalyears = $hcresult->familysurvivalyears;
            if (empty($familysurvivalyears)) {
                $familysurvivalyears = 0;
            }
            ...
            // Μόνιμη Ολική Ανικανότητα
            // Έξοδα προσαρμογής σπιτιού και λοιπός εξοπλισμός
            $houseandequipmentcost = $hcresult->houseandequipmentcost;
            ...
            // Μηνιαία αναλώσιμα, κόστος φροντίδας και αποκατάσταση
            $monthlycare = $hcresult->montlhycare;
            if (empty($monthlycare)) {
                $monthlycare = 0;
            }
            ...
            // 5η Κάρτα
            // Arithmos paidiwn pou tha spoudasoun
            $numberofchildrenuni = $hcresult->numberofchildrenuni;
            // Σπουδες σε Δημοσιο πανεπιστημιο στην πόλη που μένουν
            $dimosiosamecity = $hcresult->dimosiosamecity;
            // Σπουδες σε Δημοσιο πανεπιστημιο σε αλλη πόλη Ελλάδα
            $dimosioothercity = $hcresult->dimosioothercity;
            // Σπουδές σε ιδιωτικό Πανεπιστημιο Ελλάδα
            $privategreece = $hcresult->privategreece;
            // Σπουδες στο εξωτερικο
            $studyabroad = $hcresult->studyabroad;
            if (empty($studyabroad)) {
                $studyabroad = 0;
            }
            //Μεταπτυχιακο η διδακτορικό
            $postgraduate = $hcresult->postgraduate;
            if (empty($postgraduate)) {
                $postgraduate = 0;
            }
            $postgraduateno = number_format($postgraduate, 2);
            ...
            // 6η Κάρτα
            //Μηνιαία Συνταξη λογω χηρείας
            $monthlypensionwidow = $hcresult->monthlypensionwidow;
            if (empty($monthlypensionwidow)) {
                $monthlypensionwidow = 0;
            }
            $monthlypensionwidowno = number_format($monthlypensionwidow, 2);
            ...
            //Μηνιαία Συνταξη λογω χηρείας Έτη παροχής από το κοινωνικο ταμείο
            $monthlypensionwidowyears = $hcresult->monthlypensionwidowyears;
            //Μηνιαία σύνταξη λόγω αναπηρίας
            $monthlypensionhc = $hcresult->monthlypensionhc;
            //Μηνιαία σύνταξη λόγω αναπηρίας Έτη παροχής από το κοινωνικο ταμείο
            $monthlypensionhcyears = $hcresult->monthlypensionhcyears;
            //Αποταμίευση ή άλλα περιουσιακά στοιχεία
            $privatesavings = $hcresult->privatesavings;
            if (empty($privatesavings)) {
                $privatesavings = 0;
            }
            $privatesavingsno = number_format($privatesavings, 2);
            ...
            //Κεφάλαιο από Άλλη ασφαλεια ζωής Ατομική ή ομαδική
            $otherinsurance = $hcresult->otherinsurance;
            if (empty($otherinsurance)) {
                $otherinsurance = 0;
            }
            $otherinsuranceno = number_format($otherinsurance, 2);
            ...
            //Κεφάλαιο από άλλη Ασφάλεια ΜΟΑ
            $othermoainsurance = $hcresult->othermoainsurance;
            if (empty($othermoainsurance)) {
                $othermoainsurance = 0;
            }
            $othermoainsuranceno = number_format($othermoainsurance, 2);
            ...
            //Προσφερόμενο Εισόδημα Συγγενών
            $offeredincome = $hcresult->offeredincome;
            if (empty($offeredincome)) {
                $offeredincome = 0;
            }
            $offeredincomeno = number_format($offeredincome, 2);
            //Έτη δυνατότητας συγγενών για προσφορά εισοδήματος
            $offeredincomeyears = $hcresult->offeredincomeyears;
            // Κεφάλαιο Σύνταξης λόγω Χηρείας
            $kslx = $monthlypensionwidow * 12 * $monthlypensionwidowyears;
            $kslxno = number_format($kslx, 2);
            // Κεφάλαιο Σύνταξης λόγο Αναπηρίας
...
$data = array();
 ...
 $cur_age = $this->input->post('cur_age');
    ...
    function editclientirma($id)
    {
        ...
        public function savePensionValue()
    {

      $final_amt = $this->input->post('final_amt');
      $ins_need = $this->input->post('ins_need');
      $userId = $this->input->post('userId');
      ...
      $data= array(
        ...
        'ret_age'=>$this->input->post('ret_age'),
        'life_exp'=>$this->input->post('life_exp'),
        'mon_income'=>$this->input->post('mon_income'),
        'income_need'=>$this->input->post('income_need'),
        'per_income_need'=>$this->input->post('per_income_need'),
        'cal_income'=>$this->input->post('cal_income'),
        'mon_med_exp'=>$this->input->post('mon_med_exp'),
        'mon_cal_income'=>$this->input->post('mon_cal_income'),
        'lifestyle'=>$this->input->post('lifestyle'),
        'lifestyle_cal_income'=>$this->input->post('lifestyle_cal_income'),
        'ssb'=>$this->input->post('ssb'),
        'group_pen_prog'=>$this->input->post('group_pen_prog'),
        'cur_pen_prog'=>$this->input->post('cur_pen_prog'),     

        'create_date'=>$create_date,
      );

      $this->db->insert('pension_arc',$data);
    }

     public function saveEducationValue()
    {
      ...
      $data= array(
        ...
        'inv_need'=>$ins_need,
        'age'=>$this->input->post('age'),
        'age_need'=>$this->input->post('age_need'),
        'how_many_years'=>$this->input->post('how_many_years'),
        'cost_univer'=>$this->input->post('cost_univer'),
        'raise_cost'=>$this->input->post('raise_cost'),
        'living_cost'=>$this->input->post('living_cost'),
        'living_raise_cost'=>$this->input->post('living_raise_cost'),
        'amount_place'=>$this->input->post('amount_place'),
        'interest'=>$this->input->post('interest'),
        'sub_cal'=>$this->input->post('sub_cal'),

        'create_date'=>$create_date,
      );

      $this->db->insert('education_arc',$data);
    }
    ...
    $password = $this->input->post('p');
       ...
       $data['profile'] = $this->user_model->getRowData('members', 'id="'.$id.'"', $join_array = false);  
      $this->load->view('changepass_client',$data);
    }
